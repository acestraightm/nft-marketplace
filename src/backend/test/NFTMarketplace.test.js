...
describe("NFTMarketplace", function() {
  ...
  describe("Making marketplace items", function() {
    beforeEach(async function () {
      // addr1 mints an nft
      await nft.connect(addr1).mint(URI);
      // addr1 approves marketplace to send nft
  ...
  describe("Purchasing market items", function() {
    let price = 2;
    let totalPriceInWei;
    beforeEach(async function () {
      // addr1 mints an nft
    ...
    it("Should fail for invalid item ids, sold items and when not enough ether is paid", async function() {
      ...
      await expect(marketplace.connect(addr2).purchaseItem(1, {value: toWei(price)})).to.be
        .revertedWith("not enough ether to cover item price and market fee");
      // addr2 purcahses item 1
      await expect(marketplace.connect(addr2).purchaseItem(1, {value: totalPriceInWei}));
